...
export default function AlarmContentIcon({type} : {type: string}) {
  if (type == 'warn') {
    ...
    return(
      <DeleteOutlineOutlinedIcon color='error' sx={{width:'100px', fontSize:'4rem'}}/>
    )
  } else if (type === 'error') {
    // error의 경우
    return(
    ...
    return (
      <SensorsOffOutlinedIcon color='error' sx={{width:'100px', fontSize:'4rem'}}/>
    )
  } else {
    return(
      <SensorsOutlinedIcon sx={{width:'100px', fontSize:'4rem'}}/>
    )
  }
}